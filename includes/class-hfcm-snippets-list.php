...
class Teamone_Hfcm_Snippets_List extends WP_List_Table
{
    ...
    public static function delete_snippet( $id )
    {
        ...
        global $wpdb;
        $table_name = "{$wpdb->prefix}team_one_hfcm_scripts";
        $location = self::get_location_from_data($id);
        self::del_rediskey($location);
        $wpdb->delete(
    ...
    public function column_default( $item, $column_name )
    {
        ...
        case 'shortcode':
            return '[teamone_hfcm id="' . absint($item['script_id']) . '"]';
    ...
    function column_name( $item )
    {
        ...
        $actions = array(
            'edit'   => sprintf('<a href="?page=%s&action=%s&id=%s&_wpnonce=%s">' . esc_html__('Edit', 'header-footer-code-manager') . '</a>', esc_attr('team-one-hfcm-update'), 'edit', absint($item['script_id']), $edit_nonce),
            'copy'   => sprintf('<a href="javascript:void(0);" data-shortcode=\'[teamone_hfcm id="%s"]\'  class="hfcm_copy_shortcode" id="hfcm_copy_shortcode_%s">' . esc_html__('Copy Shortcode', 'header-footer-code-manager') . '</a>', absint($item['script_id']), absint($item['script_id'])),
    ...
    public function process_bulk_action()
    {
        ...
        if ('delete' === $this->current_action() ) {
            ...
            if (!wp_verify_nonce($nonce, 'hfcm_delete_snippet') ) {
                ...
                Team_One_NNR_HFCM::hfcm_redirect(admin_url('admin.php?page=team-one-hfcm-list'));
                return;
            }
        }
    ...
    /**
     * @desc 返回redis标识key
     * @param $location string 定位条件
     * @return string
     */
    public static function del_rediskey($location){
        global $wpdb;
        $table_name    = $wpdb->prefix . 'team_one_hfcm_scripts';

        //缓存链接层刷新
        //判断是否为短代码标识键
        if($location==CH_HFCM_REDIS_SHORTCODE_KEY){
            $nnr_hfcm_snippets_sql_key = $location;
        }else{
        ...
        if($redis->open_redis){
            ...
            $file_cache = new Teamone_Hfcm_Cache_File();
             //删除缓存文件
            $file_cache->del_json_file($rediskey);
        }
    ...
    /**
     * @desc 返回redis标识key
     * @param $string string 定位条件
     * @return string
     */
    public static function get_location_from_data($id){

        global $wpdb;
        $table_name = $wpdb->prefix .'team_one_hfcm_scripts';
         // 获取location值
         $script_data  = $wpdb->get_results(
            $wpdb->prepare( "SELECT location FROM `{$table_name}` WHERE script_id=%s", $id )
        );
        if($script_data){

             // 判断是否为空，空则设置短代码标识键
             if(empty($script_data[0]->location)){
                $script_data[0]->location = CH_HFCM_REDIS_SHORTCODE_KEY;
            }
            return $script_data[0]->location;
        }else{
            return '';
        }
    }
