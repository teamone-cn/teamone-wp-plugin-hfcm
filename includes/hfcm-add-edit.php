...
// Register the script
wp_register_script( 'team-one_hfcm_showboxes', plugins_url( 'js/nnr-hfcm-showboxes.js', dirname( __FILE__ ) ), array( 'jquery' ) );
...
wp_localize_script( 'team-one_hfcm_showboxes', 'hfcm_localize', $translation_array );

// Enqueued script with localized data.
wp_enqueue_script( 'team-one_hfcm_showboxes' );
?>
    ...
    <div class="wrap">
        ...
        if ( !empty( $_GET['message'] ) ) :
            if ( 1 === $_GET['message'] ) :
                ...
                <a href="<?php echo admin_url( 'admin.php?page=team-one-hfcm-list' ) ?>">&laquo; <?php esc_html_e( 'Back to list', 'header-footer-code-manager' ); ?></a>
        ...
        if ( $update ) :
            $hfcm_form_action = admin_url( 'admin.php?page=team-one-hfcm-request-handler&id=' . absint( $id ) );
        else :
            $hfcm_form_action = admin_url( 'admin.php?page=team-one-hfcm-request-handler' );
        endif;
        ?>
        <form method="post" action="<?php echo $hfcm_form_action ?>">
            ...
            <table class="wp-list-table widefat fixed hfcm-form-width form-table">
                ...
                <?php
                $nnr_hfcm_categories = Team_One_NNR_HFCM::hfcm_get_categories();
                $nnr_hfcm_tags       = Team_One_NNR_HFCM::hfcm_get_tags();
                ...
                <?php if ( $update ) : ?>
                    <tr>
                        ...
                        <td>
                            <p>
                                [teamone_hfcm id="<?php echo esc_html( $id ); ?>"]
                                <?php if ( $update ) :
                                    ?>
                                    <a data-shortcode='[teamone_hfcm id="<?php echo absint( $id ); ?>"]'
                    ...
                    <div class="wp-core-ui">
                        ...
                        <?php if ( $update ) :
                            ...
                            <a onclick="return nnr_confirm_delete_snippet();"
                               href="<?php echo esc_url( admin_url( 'admin.php?page=team-one-hfcm-list&action=delete&_wpnonce=' . $delete_nonce . '&snippet=' . absint( $id ) ) ); ?>"
        ...
        <div class="notification-dialog">
            <div class="file-editor-warning-content">
                ...
                <p>
                    <?php
                    if ( $update ) :
                        $hfcm_file_edit_dismiss_action = admin_url( 'admin.php?page=team-one-hfcm-update&hfcm-file-edit-notice-dismissed=1&id=' . absint( $id ) );
                    else :
                        $hfcm_file_edit_dismiss_action = admin_url( 'admin.php?page=team-one-hfcm-create&hfcm-file-edit-notice-dismissed=1' );
                    endif;
                    ?>
